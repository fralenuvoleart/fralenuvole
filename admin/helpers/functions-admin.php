...
function frl_show_admin_notices()
{
    ...
    foreach ($notices as $notice) {
        ...
        $type = isset($notice['type']) && in_array($notice['type'], ['success', 'warning', 'error', 'info'])
            ? $notice['type']
...
function frl_display_all_admin_notices()
{
    ...
    if (frl_get_option('disable_plugin')) {
        // Display Env warning
        $message[] = __('The plugin is currently disabled. Modules and frontend features are inactive
        (except category rewrites, translations, shortcodes and core features).', FRL_PREFIX);
    }
...
function frl_batch_update_options($options, $force_update = false)
{
    ...
    foreach ($options as $key => $value) {
        ...
        if (!$force_update) {
            ...
            else if (in_array($field_type, ['textarea', 'textlist'])) {
