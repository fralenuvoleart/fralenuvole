...
- **Access Control TTL Consistency:** Both [`frl_has_access()`](includes/helpers/functions-access-control.php:95) standard path and [`frl_get_auth_cookie_user_data()`](includes/mu/functions-mu-plugin.php:135) DB query use **300s TTL** — one consistent rule for all access-control caching. Early-loading access checks (before `plugins_loaded`) are handled by [`frl_mu_check_access()`](includes/mu/functions-mu-plugin.php:162), not `frl_has_access()`. All MU-plugin-specific early-loading functions are co-located in [`functions-mu-plugin.php`](includes/mu/functions-mu-plugin.php).
