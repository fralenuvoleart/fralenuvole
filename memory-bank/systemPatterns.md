...
- **Adapter files own their own companions — `translator.php` doesn't route:** `adapters/polylang.php` itself `require_once`s `adapters/polylang-admin-access.php` (Polylang-only `admin_menu` patch) at the bottom of its own file. The generic module entry point (`translator.php`) only requires the interface, the adapter, and the service — it never needs a per-plugin list of companion files; adding a new adapter later touches `translator.php` zero times for this concern.
