...
- **`frl_get_translation_adapter_class()` (`includes/helpers/functions-translator-helpers.php`) is the single source of truth for adapter selection:** returns the FQCN for the active plugin or `null`. Both `frl_is_multilingual_plugin_active()` (true iff non-null) and `Frl_Translation_Service::__construct()` (`new (frl_get_translation_adapter_class())()`) derive from it — adding a new adapter is a one-line change in this single function, never two separate places. The gate guarantees a non-null result whenever the constructor runs, so the adapter property needs no nullable type and no null-checks at any call site. Do not reintroduce a null/stub adapter class — this was tried mid-refactor and reverted in favor of fixing the gate at the source.
- **Adapter files own their own companions:** `adapters/polylang.php` itself `require_once`s `adapters/polylang-admin-access.php` (Polylang-only `admin_menu` patch) at the bottom of its own file. `translator.php` conditionally requires the top-level adapter file inline (`if ( frl_is_polylang_active() ) { require_once ...; }`) — single-call-site logic, deliberately not wrapped in its own helper function (avoid one-off wrapper functions for logic used at exactly one call site; reuse the existing detection function directly instead).
