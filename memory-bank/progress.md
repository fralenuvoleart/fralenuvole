# Project Progress — Current Feature Inventory | Last updated: 2026-09-12
...
| Rewriter | `core/rewriter/` | Feature-based URL rewriting system (CPT/taxonomy base translation and removal) with priority-ordered, self-registering features. Cache invalidation on `flush_rewrite_rules` (not just `update_option_permalink_structure`). See `docs/REWRITER.md`. |
...
| Schema | `public/schema/` | Unified JSON-LD orchestrator: single `wp_head` output, context-driven schema selection via `FRL_SCHEMA_GLOBAL_TYPES`/`FRL_SCHEMA_POST_TYPE_MAP`/`FRL_SCHEMA_SPECIAL_PAGES`, per-type `_if` toggles, `@source` system for textlist arrays, recursive builder with placeholder + field-name resolution, term/person enrichment from WP data, translation pass via `FRL_SCHEMA_TRANSLATE_KEYS`. No SASWP dependency. Legacy preserved in `public/schema-legacy/`. |
...
| `wsform` | WS Form language pre-fill, form-submission webhook dispatch (async via WP-Cron by default; all envs now use `use_cron: true`), spam filter, submission stats widget. Dedup key includes webhook URL (multi-destination safe). Channel-tracking and CTA click handling fully extracted to shared `public/channel-tracking.php` infra and standalone `call_to_actions` module respectively. Phone sanitizer: explicit-international ('+'/'00'/'011') numbers are pattern-gated against the union of configs sharing the resolved country code (pattern-less codes keep length-only checks); bare-path detection tries all same-code configs before rejecting. Test: `tests/phone-pattern-gate-test.php`. Blanked-phone email-only admin notification (includes submitter email), gated behind `wsform_phone_invalid_notify` (default off). |
...
- **`FRL_MODE=disable` guard position in `fralenuvole.php` is load-bearing.** Must precede any `FRL_DIR_PATH` usage — the constant is defined in `config/config-base.php`, loaded by `bootstrap.php` AFTER the disable early-return. Placing the guard after a `require_once` that references `FRL_DIR_PATH` causes a fatal error. Fixed 2026-07-14.
