...
- MU plugin `pre_option_cron` filter removes orphaned cron events (unregistered schedules) during WP Cron to prevent `invalid_schedule` errors. Also sanitizes `args` to array to prevent `TypeError` on null args. **Fix applied (v5.4.0):** Cron filter registration moved before the exclusion-settings early return in `frl_plugins_exclusion_filter()` so it always runs during WP Cron even when all exclusion settings are disabled.
