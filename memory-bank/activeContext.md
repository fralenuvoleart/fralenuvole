...
- **Version key is never touched:** The `foreach` loop uses `!is_numeric($timestamp)` guard — any non-numeric top-level key (e.g. `version`, any future metadata) passes through completely untouched. This prevents the `_upgrade_cron_array()` corruption cycle without requiring explicit version preservation. The previous approach (rebuilding the array + conditional `isset($cron['version'])` preservation) was fragile — corrupt data missing the version key would still perpetuate corruption. In-place modification with numeric guard is the robust solution.
