...
require_once __DIR__ . '/../modules/wsform/config-constants-wsform.php';
...
function frl_wsf_maybe_prepend_country_code( string $digits ): array {
	...
	// is already present without a leading '+'.
	foreach ( PHONE_COUNTRY_CONFIGS as $config ) {
		$code     = $config['code'];
...
function frl_wsf_sanitize_phone_number(
	...
	// Strip trailing extension markers so their digits don't merge.
...
function frl_wsf_sanitize_phone_numbers_batch(
	...
	return array_map(
		function ( $raw ) use ( $convert_vanity_letters ) {
			return frl_wsf_sanitize_phone_number( $raw, $convert_vanity_letters );
		},
		$raw_list
	);
}
...
// =====================================================================
// 2. frl_wsf_maybe_prepend_country_code() — UNIT TESTS
// =====================================================================
echo "\n=== 2. frl_wsf_maybe_prepend_country_code() ===\n";

// 2a. Empty string
$r = frl_wsf_maybe_prepend_country_code( '' );
...
$r = frl_wsf_maybe_prepend_country_code( '0555123456' );
...
$r = frl_wsf_maybe_prepend_country_code( '995555123456' );
...
$r = frl_wsf_maybe_prepend_country_code( '099555123456' );
assert_false( 'GE with trunk+code (0995...): not matched', $r['prepended'] );

// 2g. SA mobile — no trunk
$r = frl_wsf_maybe_prepend_country_code( '0821234567' );
...
$r = frl_wsf_maybe_prepend_country_code( '27821234567' );
...
$r = frl_wsf_maybe_prepend_country_code( '0211234567' );
...
$r = frl_wsf_maybe_prepend_country_code( '0111234567' );
...
$r = frl_wsf_maybe_prepend_country_code( '0311234567' );
...
$r = frl_wsf_maybe_prepend_country_code( '995' );
...
$r = frl_wsf_sanitize_phone_number( '+995 555 123 456' );
...
$r = frl_wsf_sanitize_phone_number( '+27 82 123 4567' );
...
$r = frl_wsf_sanitize_phone_number( '00995 555 123 456' );
...
$r = frl_wsf_sanitize_phone_number( '011 44 20 7946 0958' );
...
$r = frl_wsf_sanitize_phone_number( '021 123 4567' );
...
$r = frl_wsf_sanitize_phone_number( '031 123 4567' );
...
$r = frl_wsf_sanitize_phone_number( '082 123 4567 x99' );
...
$r = frl_wsf_sanitize_phone_number( '555 123 456 #42' );
...
$r = frl_wsf_sanitize_phone_number( '1-800-FLOWERS' );
...
$r = frl_wsf_sanitize_phone_number( '12345' );
...
$r = frl_wsf_sanitize_phone_number( '0123456789' );
...
$r = frl_wsf_sanitize_phone_number( '+44 (0) 20 7946 0958' );
...
$r = frl_wsf_sanitize_phone_number( '0044 (0) 20 7946 0958' );
...
$r = frl_wsf_sanitize_phone_number( '011 555 1234' );
...
// =====================================================================
// 4. frl_wsf_sanitize_phone_numbers_batch()
// =====================================================================
echo "\n=== 4. frl_wsf_sanitize_phone_numbers_batch() ===\n";

$batch = frl_wsf_sanitize_phone_numbers_batch( array( '555123456', '0821234567', null, '' ) );
...
// =====================================================================
// 5. frl_wsf_split_multiple_phone_numbers()
// =====================================================================
echo "\n=== 5. frl_wsf_split_multiple_phone_numbers() ===\n";

$split = frl_wsf_split_multiple_phone_numbers( '555123456 / 0821234567' );
...
$split = frl_wsf_split_multiple_phone_numbers( '555123456 or 0821234567' );
assert_eq( 'Split "or": count', 2, count( $split ) );

$split = frl_wsf_split_multiple_phone_numbers( '555123456 and 0821234567' );
assert_eq( 'Split "and": count', 2, count( $split ) );

// Comma with extension should NOT split
$split = frl_wsf_split_multiple_phone_numbers( '555123456, ext. 42' );
...
for ( $i = 0; $i < 1000; $i++ ) {
	frl_wsf_sanitize_phone_number( '555123456' );
	frl_wsf_sanitize_phone_number( '0821234567' );
	frl_wsf_sanitize_phone_number( '+995555123456' );
}
