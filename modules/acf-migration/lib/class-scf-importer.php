...
class Frl_Scf_Importer {
    ...
    public function get_repeater_configs(): array { return $this->repeater_configs; }
    public function get_field_key_map(): array    { return $this->field_key_map; }
    public function get_log_entry(): array         { return $this->log_entry; }

    // ─── Group import ───────────────────────────────────────────

    private function import_groups(): void {
        $groups = $this->ufj['groups'] ?? [];
    ...
    private function import_fields(int $group_id, array $group_data, string $group_key): void {
        $boxes = $group_data['boxes'] ?? [];
        $order = 0;
        foreach ($boxes as $box) {
            foreach (($box['fields'] ?? []) as $field_data) {
    ...
    private function import_single_field(int $group_id, array $field_data, int $menu_order, string $group_key): void {
        ...
        if (!$this->dry_run) {
            $this->save_field_map($field_name, $field_key, $group_key,
                $field_data['type'] ?? 'text', false, '');
        }
    ...
    private function write_field_postmeta(int $field_id, array $field_data, string $field_key): void {
        ...
        $this->add_postmeta($field_id, 'wrapper', [
            'width' => (string) ($field_data['width'] ?? ''),
            'class' => '', 'id' => '',
        ]);
        $cond = $field_data['conditional_logic'] ?? [];
        $this->add_postmeta($field_id, 'conditional_logic',
            is_array($cond) && !empty($cond) ? [$cond] : 0);
    ...
    private function import_repeater_sub_fields(int $parent_id, array $field_data, string $parent_name, string $group_key): void {
        ...
        foreach ($sub_fields as $sub) {
            ...
            if (!$this->dry_run) {
                $this->save_field_map($composite, $sub_key, '',
                    $sub['type'] ?? 'text', false, $parent_name);
            }

            $sub_configs[$sub_name] = ['key' => $sub_key, 'type' => $sub['type'] ?? 'text'];
        ...
        if (!$this->dry_run) {
            $this->save_field_map($parent_name,
                $this->field_key_map[$parent_name] ?? '',
                '', 'repeater', true, '');
        }

        $this->repeater_configs[$parent_name] = [
            'name'       => $parent_name,
    ...
    private function import_option_pages(): void {
        $pages = $this->ufj['option_pages'] ?? [];
        if (empty($pages) || $this->dry_run || !function_exists('acf_add_options_page')) { return; }
        foreach ($pages as $page) {
            ...
            if (!empty($page['position'])) { $args['position'] = (int) $page['position']; }
            if (!empty($page['icon']))     { $args['icon_url']  = $page['icon']; }
            if (!empty($page['parent']))   { $args['parent_slug'] = $page['parent']; }
    ...
    private function save_field_map(string $field_name, string $field_key, string $group_key, string $field_type, bool $is_repeater, string $parent_name): void {
        ...
        $wpdb->query($wpdb->prepare(
            ...
            $field_name, $field_key, $field_type, $group_key, $is_repeater ? 1 : 0, $parent_name
        ));
    }
    ...
    private function add_postmeta(int $post_id, string $key, $value): void {
        ...
        if ($meta_id && !is_wp_error($meta_id)) {
            ...
            error_log(sprintf(
                ...
                $post_id, $key, $meta_id->get_error_message()
            ));
        }
    }
}
