...
class Frl_Acpt_Parser {
    ...
    private function convert_meta_group(array $meta): ?array {
        ...
        $group = [
            ...
            'location' => $this->convert_location_rules($meta['belongs'] ?? []),
            'boxes'    => [],
        ];
    ...
    public function convert_field(array $field): ?array {
        ...
        // Conditionals
        $conditions = $field['visibilityConditions'] ?? [];
        if (!empty($conditions)) {
     ...
     * @return mixed
     */
    private function extract_advanced_option(array $field, string $key, $default = '') {
        $opts = $field['advancedOptions'] ?? [];
        if (!is_array($opts)) {
            return $default;
        }
