...
function frl_wsf_clear_field_249_errors($field_error_action_array, $post_mode, $submit)
{
    ...
    $filtered_errors = array_filter($field_error_action_array, function($error) {
...
function frl_wsf_execute_webhook_submission($args)
{
    ...
    if (empty($webhook_url) || !filter_var($webhook_url, FILTER_VALIDATE_URL)) {
        frl_log('WEBHOOK ERROR: frl_wsf_execute_webhook_submission() - Invalid or missing webhook URL.', ['url' => $webhook_url]);
        return;
    }
...
function frl_wsf_spam_filter_submission($field_error_action_array, $post_mode, $submit)
{
    ...
    foreach ($configs as $webhook_config) {
        ...
        $spam_filter = $webhook_config['spam_filter'] ?? [];
        if (empty($spam_filter)) {
            continue;
        }

        // Block if ALL specified spam indicator fields are filled (not empty)
        $fields_to_check = $spam_filter['block_if_all_filled'] ?? [];
